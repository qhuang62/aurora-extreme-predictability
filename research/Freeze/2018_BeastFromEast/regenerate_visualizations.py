"""
Regenerate Beast from East Stage 1 visualizations (FIXED VERSION).
...
CONFIG = {
    ...
    # Two regions: smaller for temp, larger for synoptic patterns
    'region_temp': {
        'name': 'Europe & North Atlantic',
        'lat_min': 35.0,
        'lat_max': 75.0,
        'lon_min': -30.0,
        'lon_max': 40.0
    ...
    'target_dates': {
        'onset': '2018-02-27 00:00',
        'peak': '2018-02-28 00:00',
        'recovery': '2018-03-02 00:00'
    },
...
def load_era5_data():
    ...
    surf_ds = xr.open_dataset(data_dir / 'beast_2018_surface_feb03-mar02.nc')
    atmos_ds = xr.open_dataset(data_dir / 'beast_2018_atmospheric_feb03-mar02.nc')
...
def regenerate_for_lead_time(lead_days, truth_ds):
    ...
    for phase_name, phase_info in phases.items():
        ...
        output_path = output_dir / f"beast_{lead_days}day_{phase_name}_t2m.png"
        ...
        plot_elegant_comparison(
            ...
            title=f'Beast from East 2018 - {lead_days}-day lead | Valid: {pd.Timestamp(valid_time).strftime("%Y-%m-%d %H:%M UTC")}',
        ...
        output_path = output_dir / f"beast_{lead_days}day_{phase_name}_t850.png"
        plot_elegant_comparison(
            ...
            title=f'Beast from East 2018 - {lead_days}-day lead | Valid: {pd.Timestamp(valid_time).strftime("%Y-%m-%d %H:%M UTC")}',
            save_path=output_path,
            vmin=-20, vmax=10,  # Adjusted for 850 hPa range
            cmap='RdBu_r',
            extent=extent
        )
        print(f"      ✓ {output_path.name}")
        ...
        output_path = output_dir / f"beast_{lead_days}day_{phase_name}_z500.png"
        plot_elegant_comparison(
        ...
        output_path = output_dir / f"beast_{lead_days}day_{phase_name}_msl.png"
        plot_elegant_comparison(
...
def main():
    """Main routine."""
    print("="*80)
    print("Beast from East Stage 1 - Visualization Regeneration (v2 - FIXED)")
    print("="*80)
