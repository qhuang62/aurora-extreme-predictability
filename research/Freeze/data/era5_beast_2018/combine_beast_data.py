#!/usr/bin/env python3
"""
Combine Feb 3 - Mar 2, 2018 ERA5 data into single files with continuous timesteps.

After running this, the combined files will have ~112 timesteps:
  Feb 3-28:  26 days × 4 = 104 timesteps
  Mar 1-2:   2 days × 4 = 8 timesteps
  Total:     112 timesteps

This enables:
  • Stage 1: All 4 lead times (1, 7, 14, 21 days) targeting Feb 24 onset
    - 21-day: Init Feb 3, 12 UTC (time_idx 2)
    - 14-day: Init Feb 10, 12 UTC (time_idx 30)
    - 7-day: Init Feb 17, 12 UTC (time_idx 58)
    - 1-day: Init Feb 23, 12 UTC (time_idx 82)
  • Full verification: Feb 22 - Mar 2 (onset → peak → recovery)
    - Feb 24: Onset day (time_idx 86)
    - Mar 1: Peak day (time_idx 106)
    - Mar 2: Recovery assessment (time_idx 110)
"""
...
def combine_data():
    """Combine Feb 3 - Mar 2, 2018 data files into single continuous files."""

    data_dir = Path(__file__).parent

    print("=" * 80)
    print("  Combining Feb 3 - Mar 2, 2018 ERA5 Data for Beast from the East")
    print("=" * 80)
    print()

    # Generate date range (Feb 3 - Mar 2, 2018)
    ...
    if len(surface_files) < 2:
        ...
        print("Run download_beast_complete.py first to download all dates.")
        return
    ...
    # Save
    output_surf = data_dir / "beast_2018_surface_feb03-mar02.nc"
    ...
    # Save
    output_atmos = data_dir / "beast_2018_atmospheric_feb03-mar02.nc"
    ...
    # Find Feb 3, Feb 10, Feb 17, Feb 23 at 12 UTC
    for i, t in enumerate(verify_surf.valid_time.values):
        t_dt = str(t)
        if ('2018-02-03' in t_dt and '12:00' in t_dt):
            print(f"     [{i:2d}] {t} ← 21-day lead init")
        elif ('2018-02-10' in t_dt and '12:00' in t_dt):
            print(f"     [{i:2d}] {t} ← 14-day lead init")
        elif ('2018-02-17' in t_dt and '12:00' in t_dt):
            print(f"     [{i:2d}] {t} ← 7-day lead init")
        elif ('2018-02-23' in t_dt and '12:00' in t_dt):
            print(f"     [{i:2d}] {t} ← 1-day lead init")
        elif '2018-02-24' in t_dt and '00:00' in t_dt:
            print(f"     [{i:2d}] {t} ← ONSET (Feb 24 00 UTC)")
        elif '2018-03-01' in t_dt and '12:00' in t_dt:
            print(f"     [{i:2d}] {t} ← PEAK (Mar 1 12 UTC)")
    ...
    print(f"  • Date range: Feb 3 - Mar 2, 2018")
    print(f"  • Output files:")
    print(f"      {output_surf.name}")
    print(f"      {output_atmos.name}")
    print()
    print("Now you can run Stage 1 analysis:")
    print("  • 21-day lead: Init Feb 3, 12 UTC → Predict Feb 24 onset")
    print("  • 14-day lead: Init Feb 10, 12 UTC → Predict Feb 24 onset")
    print("  • 7-day lead: Init Feb 17, 12 UTC → Predict Feb 24 onset")
    print("  • 1-day lead: Init Feb 23, 12 UTC → Predict Feb 24 onset")
    print()
