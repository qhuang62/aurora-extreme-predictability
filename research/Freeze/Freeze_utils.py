"""
Freeze/Cold Wave specific utilities.

This module contains functions specific to freeze event analysis:
- Regional subset extraction
- Spatial statistics (mean, min, max over region)
- Temporal detection (onset, peak, recovery)
- Freeze-specific metrics (RMSE, spatial extent, IoU, duration)
- Freeze-specific visualizations
"""
...
def detect_onset_timing(t2m_mean, times, threshold=0.0):
    """
    Detect onset timing (first time regional mean crosses threshold).
    ...
    -------
    datetime or None
        Onset time, or None if threshold never crossed
    """
    below_threshold = np.where(np.array(t2m_mean) < threshold)[0]

    if len(below_threshold) > 0:
        onset_idx = below_threshold[0]
        return times[onset_idx]
    else:
        return None


def detect_peak_timing(t2m_mean, times):
    """
    Detect peak cold timing (minimum regional mean temperature).
    ...
    """
    peak_idx = np.argmin(t2m_mean)
    return times[peak_idx], float(t2m_mean[peak_idx])


def detect_recovery_timing(t2m_mean, times, threshold=0.0):
    """
    Detect recovery timing (last time regional mean below threshold).
    ...
    -------
    datetime or None
        Recovery time (first timestep after last below-threshold), or None
    """
    below_threshold = np.where(np.array(t2m_mean) < threshold)[0]

    if len(below_threshold) > 0:
        last_idx = below_threshold[-1]
        # Recovery is the next timestep after last below-threshold
...
def compute_spatial_extent(field, thresholds):
    """
    Compute spatial extent (fraction of region) below temperature thresholds.
    ...
    thresholds : list of float
        Temperature thresholds (e.g., [0, -5, -10])

    Returns
    -------
    dict
        Mapping from threshold to fraction of region below that threshold
    """
...
def compute_iou(forecast_field, truth_field, threshold):
    """
    Compute Intersection over Union for areas below threshold.
...
def compute_freeze_rmse(forecast_fields, truth_fields, phase_indices):
    """
...
def compute_freeze_metrics(forecast_fields, truth_ds, forecast_times,
                           region, onset_date, peak_date, recovery_date,
                           thresholds=[0, -5, -10]):
    """
    Compute comprehensive freeze event metrics.
    ...
    thresholds : list
        Temperature thresholds for spatial extent
    ...
    # forecast_fields are global (720, 1440), need to subset to region
    import xarray as xr

    # Create DataArray from forecast fields for easy subsetting
    # Aurora 0.25° grid: 720 lats (90 to -89.75), 1440 lons (0 to 359.75)
    global_lats = np.linspace(90, -90, 720, endpoint=False)  # 90, 89.75, ..., -89.75
    global_lons = np.linspace(0, 360, 1440, endpoint=False)  # 0, 0.25, ..., 359.75
    ...
    # Detect timing in forecast
    forecast_onset = detect_onset_timing(forecast_stats['mean'], forecast_times, threshold=0.0)
    forecast_peak_time, forecast_peak_temp = detect_peak_timing(forecast_stats['mean'], forecast_times)
    forecast_recovery = detect_recovery_timing(forecast_stats['mean'], forecast_times, threshold=0.0)

    # Detect timing in truth
    truth_times = truth_t2m.valid_time.values
    truth_onset = detect_onset_timing(truth_stats['mean'], truth_times, threshold=0.0)
    truth_peak_time, truth_peak_temp = detect_peak_timing(truth_stats['mean'], truth_times)
    truth_recovery = detect_recovery_timing(truth_stats['mean'], truth_times, threshold=0.0)
    ...
    rmse_results = compute_freeze_rmse(forecast_fields_regional, truth_t2m, phase_indices)

    # Compute spatial extent and IoU at peak (using regional subset)
    peak_idx = phase_indices['peak']
    if peak_idx < len(forecast_fields_regional):
        forecast_extents = compute_spatial_extent(forecast_fields_regional[peak_idx], thresholds)
        truth_peak = truth_t2m.isel(valid_time=peak_idx).values
        truth_extents = compute_spatial_extent(truth_peak, thresholds)

        iou_scores = {}
        for thresh in thresholds:
            iou_scores[thresh] = compute_iou(forecast_fields_regional[peak_idx], truth_peak, thresh)
    ...
    # Duration (number of timesteps below 0°C)
    forecast_duration = np.sum(forecast_stats['mean'] < 0) * 6  # hours
    truth_duration = np.sum(truth_stats['mean'] < 0) * 6  # hours
...
def save_freeze_results(results, output_path):
    """
    Save freeze stage 1 results to JSON file.
...
def plot_freeze_temporal_evolution(results, output_dir, event_name="Freeze Event"):
    """
    ...
    colors = {1: 'blue', 7: 'green', 14: 'orange', 21: 'red'}
    ...
    plt.tight_layout()
    output_file = output_dir / "freeze_temporal_evolution.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"✓ Saved: {output_file.name}")
    plt.close()


def plot_spatial_comparison(forecast_field, truth_field, region, valid_time,
    ...
    if variable_name == '2m Temperature':
        ax1.contour(lons_plot, lats, forecast_field, levels=[0],
    ...
    im2 = ax2.pcolormesh(lons_plot, lats, truth_field,
                         ...
                         transform=ccrs.PlateCarree(), zorder=2)
    if variable_name == '2m Temperature':
        ax2.contour(lons_plot, lats, truth_field, levels=[0],
                   colors='black', linewidths=2, transform=ccrs.PlateCarree())
...
def plot_freeze_extent_comparison(forecast_field, truth_field, region, valid_time,
                                  lead_days, output_path, thresholds=[0, -5, -10]):
    """
    Plot freeze extent with multiple temperature contours.
    ...
    output_path : Path
        Output file
    thresholds : list
        Temperature thresholds to contour
    """
    import matplotlib.pyplot as plt
    import cartopy.crs as ccrs
    import cartopy.feature as cfeature
    ...
    # Panel 1: Aurora Forecast with freeze contours
    ax1 = plt.subplot(1, 3, 1, projection=ccrs.PlateCarree())
    ...
    # Freeze contours
    colors = {0: 'black', -5: 'blue', -10: 'darkblue'}
    linewidths = {0: 2.5, -5: 2.0, -10: 1.5}
    for thresh in thresholds:
    ...
    # Panel 2: ERA5 Truth with freeze contours
    ax2 = plt.subplot(1, 3, 2, projection=ccrs.PlateCarree())
    ...
    # Panel 3: Difference with freeze extent overlap
    ax3 = plt.subplot(1, 3, 3, projection=ccrs.PlateCarree())
    ...
    # Show 0°C contours from both (dashed=Aurora, solid=ERA5)
    ax3.contour(lons_plot, lats, forecast_field, levels=[0],
               colors='blue', linewidths=2, linestyles='--',
               transform=ccrs.PlateCarree(), label='Aurora 0°C')
    ax3.contour(lons_plot, lats, truth_field, levels=[0],
               colors='red', linewidths=2, linestyles='-',
               transform=ccrs.PlateCarree(), label='ERA5 0°C')

    ax3.set_title('Difference & Freeze Extent Comparison', fontsize=12, fontweight='bold')
    ...
    # Super title
    time_str = pd.Timestamp(valid_time).strftime('%Y-%m-%d %H:%M UTC')
    plt.suptitle(f'Freeze Extent Analysis - Valid: {time_str}',
                fontsize=14, fontweight='bold', y=0.98)
...
def plot_lead_time_skill(results, output_dir, event_name="Freeze Event"):
    """
    ...
    if valid_peak:
        ...
        ax.plot(ld_valid, err_valid, 'o-', linewidth=2, markersize=8, color='darkgreen')
    ax.set_xlabel('Lead Time (days)')
    ...
    if valid_rmse:
        ...
        ax.plot(ld_valid, rmse_valid, 'o-', linewidth=2, markersize=8, color='darkorange')
    ax.set_xlabel('Lead Time (days)')
    ...
    ax = axes[1, 1]
    ax.plot(lead_times, duration_errors, 'o-', linewidth=2, markersize=8, color='darkred')
    ax.set_xlabel('Lead Time (days)')
    ...
    output_file = output_dir / "freeze_lead_time_skill.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"✓ Saved: {output_file.name}")
    plt.close()
