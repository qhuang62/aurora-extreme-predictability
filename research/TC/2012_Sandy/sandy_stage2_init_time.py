#!/usr/bin/env python3
"""
Hurricane Sandy 2012 - Stage 2: Initialization Sensitivity Analysis

Tests all 4 initialization times (00, 06, 12, 18 UTC) on Oct 29, 2012
using the best-performing lead time from Stage 1 (1-day lead).
...
Stage 1 Results: 3-day lead had mean error 111.9 km
"""
...
def main():
    print("=" * 75)
    print("  Hurricane Sandy 2012 - Stage 2: Initialization Sensitivity")
    print("=" * 75)
    print()
    ...
    CONFIG = {
        ...
        # All initialized on Oct 27, 2012 (3 days before landfall)
        # Each init time has different steps to reach same target (Oct 30 00 UTC)
        # Coordinates from IBTrACS v04r01 (Storm ID: 2012296N14283)
        'lead_days': 3,
        'init_date': '2012-10-27',
        'landfall_date': 'Oct 30, 2012 00:00 UTC',
        'init_times': [
            {'hour': '00', 'time_idx': 20, 'init_lat': 27.5, 'init_lon': 282.9, 'steps': 11},  # 72h = 12 steps - 1
            {'hour': '06', 'time_idx': 21, 'init_lat': 28.1, 'init_lon': 283.1, 'steps': 10},  # 66h = 11 steps - 1
            {'hour': '12', 'time_idx': 22, 'init_lat': 28.8, 'init_lon': 283.5, 'steps': 9},   # 60h = 10 steps - 1
            {'hour': '18', 'time_idx': 23, 'init_lat': 29.7, 'init_lon': 284.4, 'steps': 8},   # 54h = 9 steps - 1
        ],
    }
    ...
    if not surf_file.exists() or not atmos_file.exists():
        ...
        print(f"  {surf_file}")
        print(f"  {atmos_file}")
        print("  Run download_sandy_complete.py and combine_data.py first")
        return
    ...
    # Truncate observed track to match forecast period (Oct 27 - Oct 30)
    # This ensures observed track aligns with all forecast tracks
    earliest_init_time = datetime(2012, 10, 27, 0, 0)  # Stage 2 earliest init
    landfall_time = datetime(2012, 10, 30, 0, 0)       # Landfall target

    # Historical track segment (Oct 23 - Oct 27) to show as dashed line
    stage1_start_time = datetime(2012, 10, 23, 0, 0)   # Stage 1 earliest init

    # Find indices for forecast period (Oct 27 - Oct 30)
    ...
    # Find indices for historical segment (Oct 23 - Oct 27)
    ...
    track_plot_path = CONFIG['output_dir'] / "sandy_stage2_init_tracks.png"
    plot_track_comparison(
        forecast_tracks=forecast_tracks,
        obs_track=obs_track_truncated,  # Use truncated track (solid red)
        title=f"Hurricane Sandy - Track Comparison (Different Initialization Time) ({CONFIG['lead_days']}-Day Lead)",
        save_path=track_plot_path,
        extent=[-90, -65, 10, 45],  # Same extent as Stage 1 for vertical orientation
    ...
    landfall_time = datetime(2012, 10, 30, 0, 0)  # Target landfall time

    error_plot_path = CONFIG['output_dir'] / "sandy_stage2_init_errors.png"
    plot_error_by_valid_time(
        forecast_tracks_dict=tracks_dict,
        errors_dict=errors_dict,
        target_time=landfall_time,
        ylabel="Track Error (km)",
        title=f"Hurricane Sandy - Track Error (Different Initialization Time) ({CONFIG['lead_days']}-Day Lead)",
            ...
            if hour == '00':
                try:
                    ...
                    if surf_file.exists() and atmos_file.exists():
                        ...
                        print(f"  ✓ Loaded ERA5 verification data")
                    else:
                        print(f"  ⚠ Combined ERA5 files not found, skipping field comparison")
                        era5_surf = None
                        era5_atmos = None
    ...
    for hour in ['00', '06', '12', '18']:
        if 'landfall_field_comparison' in results[hour]:
            ...
            msl_plot = CONFIG['output_dir'] / f"sandy_init{hour}_landfall_msl.png"
            plot_field_comparison(
                ...
                title=f"Sandy {hour} UTC Init (3-day Lead): MSL at Landfall",
                save_path=msl_plot,
                extent=LANDFALL_REGION
            )

            # Wind field comparison
            wind_plot = CONFIG['output_dir'] / f"sandy_init{hour}_landfall_wind.png"
            plot_field_comparison(
                ...
                title=f"Sandy {hour} UTC Init (3-day Lead): Wind at Landfall",
                save_path=wind_plot,
                extent=LANDFALL_REGION,
                cmap='YlOrRd'
            )

    # Close combined files
