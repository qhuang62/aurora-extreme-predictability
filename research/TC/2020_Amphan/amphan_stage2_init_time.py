#!/usr/bin/env python3
"""
Cyclone Amphan 2020 - Stage 2: Initialization Sensitivity Analysis

Tests all 4 initialization times (00, 06, 12, 18 UTC) on May 17, 2020
...
def main():
    print("=" * 75)
    print("  Cyclone Amphan 2020 - Stage 2: Initialization Sensitivity")
    print("=" * 75)
    print()
    ...
    CONFIG = {
        ...
        # Test 4 initialization times: 00, 06, 12, 18 UTC on May 17
        # All initialized on May 17, 2020 (3 days before landfall)
        # Following Sandy/Ian/Hinnamnor pattern: steps = (total_hours / 6) - 1
        # All forecasts end at May 20 06:00 UTC (6h before 12:00 UTC target)
        # Target marker shown at 12:00 UTC (Bangladesh landfall)
        # Coordinates from IBTrACS v04r01 (Storm ID: 2020136N10088)
        'lead_days': 3,
        'init_date': '2020-05-17',
        'landfall_date': 'May 20, 2020 12:00 UTC',  # Target marker (6h after last forecast)
        'init_times': [
            {'hour': '00', 'time_idx': 16, 'init_lat': 11.20, 'init_lon': 86.10, 'steps': 13},  # May 17 00:00 → May 20 06:00 (78h)
            {'hour': '06', 'time_idx': 17, 'init_lat': 11.50, 'init_lon': 86.20, 'steps': 12},  # May 17 06:00 → May 20 06:00 (72h)
            {'hour': '12', 'time_idx': 18, 'init_lat': 11.90, 'init_lon': 86.20, 'steps': 11},  # May 17 12:00 → May 20 06:00 (66h)
            {'hour': '18', 'time_idx': 19, 'init_lat': 12.50, 'init_lon': 86.40, 'steps': 10},  # May 17 18:00 → May 20 06:00 (60h)
        ],
    }
    ...
    if not surf_file.exists() or not atmos_file.exists():
        ...
        print(f"  {surf_file}")
        print(f"  {atmos_file}")
        print("  Run download_amphan_complete.py and combine_amphan_data.py first")
        return
    ...
    # Truncate observed track to match forecast period (May 17 - May 20)
    earliest_init_time = datetime(2020, 5, 17, 0, 0)  # Stage 2 earliest init
    landfall_time = datetime(2020, 5, 20, 12, 0)      # Target: 12:00 UTC (landfall)

    # Historical track segment (May 13 - May 17) to show as dashed line
    stage1_start_time = datetime(2020, 5, 13, 12, 0)  # Stage 1 earliest init

    # Find indices for forecast period (May 17 - May 20)
    ...
    # Find indices for historical segment (May 13 - May 17)
    ...
    track_plot_path = CONFIG['output_dir'] / "amphan_stage2_init_tracks.png"
    plot_track_comparison(
        forecast_tracks=forecast_tracks,
        obs_track=obs_track_truncated,  # Use truncated track (solid red)
        title=f"Cyclone Amphan - Track Comparison (Different Initialization Time) ({CONFIG['lead_days']}-Day Lead)",
    ...
    error_plot_path = CONFIG['output_dir'] / "amphan_stage2_init_errors.png"
    plot_error_by_valid_time(
        forecast_tracks_dict=tracks_dict,
        errors_dict=errors_dict,
        target_time=landfall_time,
        ylabel="Track Error (km)",
        title=f"Cyclone Amphan - Track Error (Different Initialization Time) ({CONFIG['lead_days']}-Day Lead)",
            ...
            if hour == '00':
                try:
                    ...
                    if surf_file.exists() and atmos_file.exists():
                        ...
                        print(f"  ✓ Loaded ERA5 verification data (May 20 12:00 UTC)")
                    else:
    ...
    for hour in ['00', '06', '12', '18']:
        if 'landfall_field_comparison' in results[hour]:
            ...
            msl_plot = CONFIG['output_dir'] / f"amphan_init{hour}_landfall_msl.png"
            plot_field_comparison(
                ...
                title=f"Amphan {hour} UTC Init (3-day Lead): MSL at Landfall",
                save_path=msl_plot,
                extent=LANDFALL_REGION
            )

            # Wind field comparison
            wind_plot = CONFIG['output_dir'] / f"amphan_init{hour}_landfall_wind.png"
            plot_field_comparison(
                ...
                title=f"Amphan {hour} UTC Init (3-day Lead): Wind at Landfall",
                save_path=wind_plot,
                extent=LANDFALL_REGION,
                cmap='YlOrRd'
            )
