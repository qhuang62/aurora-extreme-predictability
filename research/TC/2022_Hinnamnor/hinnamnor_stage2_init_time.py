#!/usr/bin/env python3
"""
Typhoon Hinnamnor 2022 - Stage 2: Initialization Sensitivity Analysis

Tests all 4 initialization times (00, 06, 12, 18 UTC) on Sep 3, 2022
...
def main():
    print("=" * 75)
    print("  Typhoon Hinnamnor 2022 - Stage 2: Initialization Sensitivity")
    print("=" * 75)
    print()
    ...
    CONFIG = {
        ...
        # Test 4 initialization times: 00, 06, 12, 18 UTC on Sep 3
        # All initialized on Sep 3, 2022 (3 days before landfall)
        # Following Sandy/Ian pattern: steps = (total_hours / 6) - 1
        # All forecasts end at Sep 6 12:00 UTC (6h before 18:00 UTC target)
        # Target marker shown at 18:00 UTC (South Korea landfall)
        # Coordinates from IBTrACS v04r01 (Storm ID: 2022239N22150)
        'lead_days': 3,
        'init_date': '2022-09-03',
        'landfall_date': 'Sep 6, 2022 18:00 UTC',  # Target marker (6h after last forecast)
        'init_times': [
            {'hour': '00', 'time_idx': 16, 'init_lat': 23.00, 'init_lon': 124.70, 'steps': 14},  # Sep 3 00:00
            {'hour': '06', 'time_idx': 17, 'init_lat': 23.70, 'init_lon': 124.70, 'steps': 13},  # Sep 3 06:00
            {'hour': '12', 'time_idx': 18, 'init_lat': 24.30, 'init_lon': 124.80, 'steps': 12},  # Sep 3 12:00
            {'hour': '18', 'time_idx': 19, 'init_lat': 25.10, 'init_lon': 124.60, 'steps': 11},  # Sep 3 18:00
        ],
    }
    ...
    if not surf_file.exists() or not atmos_file.exists():
        ...
        print(f"  {surf_file}")
        print(f"  {atmos_file}")
        print("  Run download_hinnamnor_complete.py and combine_hinnamnor_data.py first")
        return
    ...
    # Truncate observed track to match forecast period (Sep 3 - Sep 6)
    earliest_init_time = datetime(2022, 9, 3, 0, 0)  # Stage 2 earliest init
    landfall_time = datetime(2022, 9, 6, 18, 0)      # Target: 18:00 UTC (landfall)

    # Historical track segment (Aug 30 - Sep 3) to show as dashed line
    stage1_start_time = datetime(2022, 8, 30, 18, 0)  # Stage 1 earliest init

    # Find indices for forecast period (Sep 3 - Sep 6)
    ...
    # Find indices for historical segment (Aug 30 - Sep 3)
    ...
    track_plot_path = CONFIG['output_dir'] / "hinnamnor_stage2_init_tracks.png"
    plot_track_comparison(
        forecast_tracks=forecast_tracks,
        obs_track=obs_track_truncated,  # Use truncated track (solid red)
        title=f"Typhoon Hinnamnor - Track Comparison (Different Initialization Time) ({CONFIG['lead_days']}-Day Lead)",
    ...
    error_plot_path = CONFIG['output_dir'] / "hinnamnor_stage2_init_errors.png"
    plot_error_by_valid_time(
        forecast_tracks_dict=tracks_dict,
        errors_dict=errors_dict,
        target_time=landfall_time,
        ylabel="Track Error (km)",
        title=f"Typhoon Hinnamnor - Track Error (Different Initialization Time) ({CONFIG['lead_days']}-Day Lead)",
        save_path=error_plot_path,
        xaxis_hours=[0, 6, 12, 18]  # Hinnamnor Stage 2 uses all 4 UTC times
    )
            ...
            if hour == '00':
                try:
                    ...
                    if surf_file.exists() and atmos_file.exists():
                        ...
                        print(f"  ✓ Loaded ERA5 verification data (Sep 6 18:00 UTC)")
                    else:
    ...
    for hour in ['00', '06', '12', '18']:
        if 'landfall_field_comparison' in results[hour]:
            ...
            msl_plot = CONFIG['output_dir'] / f"hinnamnor_init{hour}_landfall_msl.png"
            plot_field_comparison(
                ...
                title=f"Hinnamnor {hour} UTC Init (3-day Lead): MSL at Landfall",
                save_path=msl_plot,
                extent=LANDFALL_REGION
            )

            # Wind field comparison
            wind_plot = CONFIG['output_dir'] / f"hinnamnor_init{hour}_landfall_wind.png"
            plot_field_comparison(
                ...
                title=f"Hinnamnor {hour} UTC Init (3-day Lead): Wind at Landfall",
                save_path=wind_plot,
                extent=LANDFALL_REGION,
                cmap='YlOrRd'
            )
