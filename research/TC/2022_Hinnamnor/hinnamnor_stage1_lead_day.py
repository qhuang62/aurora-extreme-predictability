#!/usr/bin/env python3
"""
Typhoon Hinnamnor 2022 - Stage 1: Lead Time Assessment

Tests 4 different lead times (1, 3, 5, 7 days) by varying initialization date.
All forecasts target the same critical event: Hinnamnor's South Korea landfall on Sep 6, 2022.

This answers: "How far in advance can we predict Hinnamnor's landfall?"

Notable features:
- 7-day lead: Initializes at peak Cat 5 intensity (Aug 30 18:00 UTC: 145 kt, 910 mb)
  Tests Aurora's ability to predict significant weakening, stalling, and sharp recurvature
- Western Pacific basin (different dynamics from Atlantic TCs)
- Out-of-sample validation (2022 > 2020 Aurora training cutoff)
"""
...
def main():
    print("=" * 75)
    print("  Typhoon Hinnamnor 2022 - Stage 1: Lead Time Assessment")
    print("=" * 75)
    print()
    print("Strategy: Variable initialization date, fixed target (Sep 6 landfall)")
    print()
    ...
    # =========================================================================
    CONFIG = {
        'event_name': 'Typhoon Hinnamnor 2022',
        'storm_name': 'HINNAMNOR',
        'storm_year': 2022,
        'landfall_date': 'Sep 6, 2022 18:00 UTC',
        ...
        # Combined data file covering Aug 30 - Sep 6
        'combined_surface_file': 'hinnamnor_2022_surface_combined.nc',
        'combined_atmos_file': 'hinnamnor_2022_atmospheric_combined.nc',
        'data_folder': 'era5_hinnamnor_2022',

        # Stage 1: Test 4 lead times, all targeting Sep 6 18:00 UTC (South Korea landfall)
        # All initialized at 18:00 UTC for exact 1/3/5/7 day leads
        # Coordinates from IBTrACS v04r01 (Storm ID: 2022239N22150)
        'lead_time_configs': [
            {
                'lead_days': 1,
                'init_date': '2022-09-05',
                'init_hour': '18',
                'time_idx': 27,  # Sep 5, 18:00 UTC in combined file (Aug 30 00:00 = 0)
                'init_lat': 34.20,  # IBTrACS: Sep 5 18:00 UTC
                'init_lon': 128.10,  # Already in °E (Western Pacific)
            },
            {
                'lead_days': 3,
                'init_date': '2022-09-03',
                'init_hour': '18',
                'time_idx': 19,  # Sep 3, 18:00 UTC in combined file
                'init_lat': 25.10,  # IBTrACS: Sep 3 18:00 UTC
                'init_lon': 124.60,  # Already in °E
            },
            {
                'lead_days': 5,
                'init_date': '2022-09-01',
                'init_hour': '18',
                'time_idx': 11,  # Sep 1, 18:00 UTC in combined file
                'init_lat': 21.30,  # IBTrACS: Sep 1 18:00 UTC (stalling phase)
                'init_lon': 125.50,  # Already in °E
            },
            {
                'lead_days': 7,
                'init_date': '2022-08-30',
                'init_hour': '18',
                'time_idx': 3,   # Aug 30, 18:00 UTC in combined file
                # Note: Aug 30 18:00 UTC was peak Cat 5 intensity (145 kt, 910 mb)
                'init_lat': 26.30,  # IBTrACS: Aug 30 18:00 UTC (peak Cat 5)
                'init_lon': 131.90,  # Already in °E
            },
        ],
    }
    ...
    print(f"Target: {CONFIG['landfall_date']} (South Korea)")
    ...
    print()
    print("Note: 7-day lead uses peak Cat 5 intensity position")
    print("      Tests Aurora's prediction of weakening + stalling + recurvature sequence")
    print()
    ...
    for config in CONFIG['lead_time_configs']:
        ...
        print(f"  Target: Sep 6 18:00 UTC landfall ({lead_days} days ahead)")
        print(f"{'=' * 75}\n")
        ...
        if not surf_file.exists() or not atmos_file.exists():
            ...
            print(f"  Run download_hinnamnor_complete.py and combine_hinnamnor_data.py first")
            continue

        # Load ERA5 data from combined files
        try:
            import xarray as xr
            static_ds, _, _ = load_era5_data(data_path, '2022-08-30')  # Load static vars
        ...
        # Run forecast (target: Sep 6 18:00 UTC)
        # Following Sandy/Ian pattern: steps = lead_days * 4 - 1
        # This makes forecasts end 6h before target (at Sep 6 12:00 UTC)
        # The tracker includes init position (t=0) + forecast steps (t=1 to N)
        #   1-day: 3 steps → 4 track points, ending at Sep 6 12:00 UTC
        #   3-day: 11 steps → 12 track points, ending at Sep 6 12:00 UTC
        #   5-day: 19 steps → 20 track points, ending at Sep 6 12:00 UTC
        #   7-day: 27 steps → 28 track points, ending at Sep 6 12:00 UTC
        steps = lead_days * 4 - 1
        ...
        preds = run_forecast(
            model, batch, tracker,
            steps=steps,
            name=f"Hinnamnor {lead_days}day",
            device=device,
            verbose=True
        )
        ...
        # Compute landfall error (Korea coast ~128°E)
        landfall_error_data = compute_landfall_error(
            forecast_track, obs_track,
            landfall_threshold_lon=128.0  # South Korea (Eastern longitude)
        )
    ...
    if len(results) > 0:
        ...
        save_path = CONFIG['output_dir'] / "hinnamnor_stage1_lead_time_results.pkl"
    ...
    if len(results) > 0:
        ...
        # Start: Aug 30 18:00 (7-day forecast initialization, earliest)
        # End: Sep 6 18:00 (target time - South Korea landfall)
        earliest_init_time = datetime(2022, 8, 30, 18, 0)  # 7-day lead init
        landfall_time = datetime(2022, 9, 6, 18, 0)  # Target: 18:00 UTC (landfall)
        ...
        track_plot = CONFIG['output_dir'] / "hinnamnor_stage1_tracks.png"
        plot_track_comparison(
            forecast_tracks=forecast_tracks,
            obs_track=obs_track_truncated,  # Use truncated track
            title="Typhoon Hinnamnor - Track Comparison (Different Lead Days)",
            save_path=track_plot,
            extent=[120, 145, 20, 50]  # Western Pacific/Korea region
        )
        ...
        if len(errors_dict) > 0:
            ...
            error_plot = CONFIG['output_dir'] / "hinnamnor_stage1_errors.png"
            plot_error_by_valid_time(
                forecast_tracks_dict=tracks_dict,
                errors_dict=errors_dict,
                target_time=landfall_time,
                ylabel="Track Error (km)",
                title="Typhoon Hinnamnor - Track Error (Different Lead Days)",
                save_path=error_plot,
                xaxis_hours=[18]  # Hinnamnor Stage 1 uses 18 UTC
            )
    ...
    # Landfall parameters (Hinnamnor landfall Sep 6, 18:00 UTC)
    # Based on experimental design: ~35°N, 128°E (South Korea)
    LANDFALL_LAT = 35.0
    LANDFALL_LON = 128.0  # Already in °E (Western Pacific)
    LANDFALL_REGION = [120, 145, 25, 50]  # Korea/Western Pacific region
    ...
    if len(results) > 0:
        for lead_days in sorted(results.keys()):
            ...
            if len(distances) > 0:
                ...
                # Load ERA5 verification data at landfall time (Sep 6 18:00 UTC)
                # Use separate Sep 6 landfall file for rigorous verification
                try:
                    import xarray as xr
                    data_path = CONFIG['base_data_path'] / CONFIG['data_folder']
                    surf_file = data_path / "hinnamnor_2022_surface_landfall.nc"
                    atmos_file = data_path / "hinnamnor_2022_atmospheric_landfall.nc"

                    if surf_file.exists() and atmos_file.exists():
                        era5_surf = xr.open_dataset(surf_file)
                        era5_atmos = xr.open_dataset(atmos_file)
                        print(f"  ✓ Loaded Sep 6 18:00 UTC ERA5 data for landfall verification")
                        ...
                        # MSL field
                        msl_plot = CONFIG['output_dir'] / f"hinnamnor_{lead_days}day_landfall_msl.png"
                        plot_field_comparison(
                            ...
                            title=f"Hinnamnor {lead_days}-day Lead: MSL at Landfall",
                            save_path=msl_plot,
                            extent=LANDFALL_REGION
                        )

                        # Wind field
                        wind_plot = CONFIG['output_dir'] / f"hinnamnor_{lead_days}day_landfall_wind.png"
                        plot_field_comparison(
                            ...
                            title=f"Hinnamnor {lead_days}-day Lead: Wind at Landfall",
                            save_path=wind_plot,
                            extent=LANDFALL_REGION,
                            cmap='YlOrRd'
                        )
                        ...
                        print(f"  ⚠ Sep 6 landfall ERA5 files not found, skipping field comparison")
