"""
Regenerate 2023 European Heatwave Stage 1 visualizations.

This script creates publication-quality spatial comparison plots for each lead time
and phase (onset, peak, recovery), showing Aurora forecasts vs ERA5 truth.
"""
...
from Heatwave_utils import extract_regional_subset
...
CONFIG = {
    ...
    # Two regions: larger for temp (smoother), even larger for synoptic patterns
    'region_temp': {
        'name': 'Western Europe (zoomed out to reduce pixelation)',
        'lat_min': 35.0,
        'lat_max': 60.0,
        'lon_min': -15.0,
        'lon_max': 25.0
    ...
    'region_synoptic': {
        'name': 'North Atlantic-Europe (full synoptic scale)',
        'lat_min': 25.0,
        'lat_max': 70.0,
        'lon_min': -40.0,
        'lon_max': 30.0
    ...
    'target_dates': {
        'onset': '2023-08-20 12:00',
        'peak': '2023-08-23 12:00',
        'recovery': '2023-08-24 18:00'
    },
...
def subset_aurora_field_to_region(global_field, region):
    ...
    """
    # Aurora 0.25° grid
    global_lats = np.arange(90, -90, -0.25)  # 720 points: 90, 89.75, ..., -89.75
    global_lons = np.arange(0, 360, 0.25)     # 1440 points: 0, 0.25, ..., 359.75
...
def plot_elegant_comparison(forecast, truth, lat, lon, variable_name, title,
                           save_path, vmin=None, vmax=None, cmap='YlOrRd', extent=None,
                           diff_cmap=None, heat_threshold=None):
    """
    Create elegant publication-quality comparison plot with country borders.

    Parameters:
    -----------
    heat_threshold : float, optional
        Temperature threshold for heatwave (e.g., 30°C). If provided, adds contour line.
    ...
    ax1.add_feature(cfeature.BORDERS.with_scale('50m'), linewidth=0.6,
    ...
    # Add heatwave threshold contour for temperature fields
    if heat_threshold is not None and ('Temperature' in variable_name or 'T2m' in variable_name or 'T850' in variable_name):
        cs1 = ax1.contour(lon, lat, forecast, levels=[heat_threshold], colors='darkred',
                          linewidths=2.5, linestyles='--', transform=ccrs.PlateCarree(), zorder=4)
        ax1.clabel(cs1, inline=True, fontsize=9, fmt=f'{heat_threshold:.0f}°C')
    ...
    ax2.add_feature(cfeature.BORDERS.with_scale('50m'), linewidth=0.6,
                    edgecolor='darkgray', linestyle='-', alpha=0.7, zorder=3)
    ...
    if heat_threshold is not None and ('Temperature' in variable_name or 'T2m' in variable_name or 'T850' in variable_name):
        cs2 = ax2.contour(lon, lat, truth, levels=[heat_threshold], colors='darkred',
                          linewidths=2.5, linestyles='--', transform=ccrs.PlateCarree(), zorder=4)
        ax2.clabel(cs2, inline=True, fontsize=9, fmt=f'{heat_threshold:.0f}°C')
    ...
    ax3.add_feature(cfeature.BORDERS.with_scale('50m'), linewidth=0.6,
                    edgecolor='darkgray', linestyle='-', alpha=0.7, zorder=3)
...
def load_era5_data():
    ...
    surf_ds = xr.open_dataset(data_dir / 'heatwave_2023_surface_jul19-aug27.nc')
    atmos_ds = xr.open_dataset(data_dir / 'heatwave_2023_atmospheric_jul19-aug27.nc')
...
def regenerate_for_lead_time(lead_days, truth_ds):
    ...
    region_temp = CONFIG['region_temp']  # Smaller for temperatures
    region_synoptic = CONFIG['region_synoptic']  # Larger for Z500/MSL
    ...
    for phase_name, phase_info in phases.items():
        ...
        output_path = output_dir / f"heatwave_{lead_days}day_{phase_name}_t2m.png"
        ...
        plot_elegant_comparison(
            ...
            title=f'2023 European Heatwave - {lead_days}-day lead | Valid: {pd.Timestamp(valid_time).strftime("%Y-%m-%d %H:%M UTC")}',
            save_path=output_path,
            vmin=15, vmax=45,  # Hot range for heatwave
            cmap='YlOrRd',  # Hot colormap
            extent=extent,
            heat_threshold=30  # 30°C heatwave threshold
        )
        ...
        output_path = output_dir / f"heatwave_{lead_days}day_{phase_name}_t850.png"
        plot_elegant_comparison(
            ...
            title=f'2023 European Heatwave - {lead_days}-day lead | Valid: {pd.Timestamp(valid_time).strftime("%Y-%m-%d %H:%M UTC")}',
            save_path=output_path,
            vmin=5, vmax=25,  # Hot range for 850 hPa
            cmap='YlOrRd',
            extent=extent,
            heat_threshold=15  # Warm air indicator at 850 hPa
        )
        print(f"      ✓ {output_path.name}")

        # --- Z500 (use synoptic region) ---
        ...
        output_path = output_dir / f"heatwave_{lead_days}day_{phase_name}_z500.png"
        plot_elegant_comparison(
        ...
        output_path = output_dir / f"heatwave_{lead_days}day_{phase_name}_msl.png"
        plot_elegant_comparison(
...
def main():
    """Main routine."""
    print("="*80)
    print("2023 European Heatwave Stage 1 - Visualization Regeneration")
    print("="*80)
